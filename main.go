...
func main() {
	...
	case "log":
		handleLog(os.Args[2:])
	case "checkout": // New case for the checkout command
...
func printUsage() {
	...
	fmt.Println("  checkout      Switch to a different branch")
}
...
// mergeConfigs merges the local and global configuration values.
...
// handleCheckout processes the `checkout` command to switch branches.
func handleCheckout(args []string) {
	if len(args) < 1 {
		fmt.Println("Please provide a branch name to checkout")
		return
	}
	branch := args[0]
	repoPath := "." // assuming current directory as the repository path

	// Call the Checkout function from commands package
	err := commands.Checkout(repoPath, branch)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}
...
// handleLog processes the `log` command to display the commit history.
func handleLog(args []string) {
	// Here we assume the repo path is the current directory.
	repoPath := "."

	// Call the Log function from commands package
	err := commands.Log(repoPath)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
	}
}

// handleHashObject processes the `hash-object` command.
...
// handleCatFile processes the `cat-file` command.
...
// handleCommit processes the `commit` command.
